...
public class SettingsFragment extends PreferenceFragmentCompat
    ...
    private static final String SETTINGS_CROSS_PROFILE_FILE_CHOOSER =
            "settings_cross_profile_file_chooser";
    private static final String SETTINGS_BLOCK_CONTACTS_SEARCHING =
            "settings_block_contacts_searching";
    private static final String SETTINGS_AUTO_FREEZE_SERVICE =
            "settings_auto_freeze_service";
    private static final String SETTINGS_AUTO_FREEZE_DELAY =
    ...
    private static final int[] AUTO_FREEZE_DELAY_SECONDS =
            new int[]{0, 60, 2 * 60, 5 * 60};
    ...
    public void onCreatePreferences(
        ...
        mServiceWork =
                IShelterService.Stub.asInterface(
                        ((Bundle) getActivity()
                                .getIntent()
        ...
        // Allow install or uninstall app
        ...
        Preference resetTrustAgents =
                findPreference(
                        "settings_reset_parent_trust_agents"
                );

        if (resetTrustAgents != null) {

            resetTrustAgents.setOnPreferenceClickListener(
                    preference -> {

                        Utility.resetParentTrustAgents(
                                requireContext()
                        );
        ...
        Preference resetFingerprint =
                findPreference(
                        "settings_reset_parent_fingerprint"
                );

        if (resetFingerprint != null) {

            resetFingerprint.setOnPreferenceClickListener(
                    preference -> {

                        Utility.resetParentFingerprint(
                                requireContext()
                        );
        ...
        Preference resetFace =
                findPreference(
                        "settings_reset_parent_face"
                );

        if (resetFace != null) {

            resetFace.setOnPreferenceClickListener(
                    preference -> {

                        Utility.resetParentFace(
                                requireContext()
                        );
        ...
        Preference resetIris =
                findPreference(
                        "settings_reset_parent_iris"
                );

        if (resetIris != null) {

            resetIris.setOnPreferenceClickListener(
                    preference -> {

                        Utility.resetParentIris(
                                requireContext()
                        );
        ...
        if (Build.VERSION.SDK_INT == Build.VERSION_CODES.Q) {
            ...
            mPrefCrossProfileFileChooser.setEnabled(
                    false
            );
        }
    ...
    private void updateAutoFreezeDelay() {
        ...
        mPrefAutoFreezeDelay.setSummary(
                getString(
                        R.string.format_minutes,
                        mManager.getAutoFreezeDelay() / 60
                )
        );
    }

    private boolean openSummaryUrl(
            Preference pref) {
    ...
    public boolean onPreferenceChange(
        ...
        if (preference ==
            ...
            if (!enabled) {
                ...
                mManager.setCrossProfileFileChooserEnabled(
                        false
                );
            ...
            if (Build.VERSION.SDK_INT >=
                ...
                boolean hasPermission =
                        ensureSpecialAccessPermission(
                                ...
                                R.string.request_storage_manager,
                ...
                // because File Shuttle needs to start activities
                // in the background
                //
                // We cannot do the same notification trick as in
                // initial setup because it would be too annoying
                // having to click a notification every time a user
                // tries to use File Shuttle.
                //
                // NOTE:
                // Enabling this permission may mask some bugs with
                // background activities. Always test with this disabled.
            ...
            mManager.setBlockContactsSearchingEnabled(
                    (boolean) newState
            );
            ...
            mManager.setWorkToPersonalClipboardEnabled(
                    (boolean) newState
            );
            ...
            mManager.setAutoFreezeServiceEnabled(
                    (boolean) newState
            );
    ...
    private boolean ensureSpecialAccessPermission(
            CheckPermissionCallback checkPermission,
            int alertRes,
